pub mod weighted_graph{
    ...
    impl Edge {
        //Constructor, Time O(1) Space O(1)
        pub fn new(neighbor: i32, weight: i32) -> Self {
    ...
    impl<T: std::hash::Hash + Eq + Clone> WeightedGraph<T> {
        ...
        pub fn newFromFile(&self , filename: &String) -> WeightedGraph<String>{
            let mut my_graph = WeightedGraph::<String>::ler(filename);
            my_graph
        }


        pub fn add_edge(&mut self, a: i32, b: i32, w: i32){
            self.adj
                .entry(a.clone())
        ...
        pub fn rotulo(&self, node_id: i32) -> &T {
            let node:&Node<T> = &self.nodes[node_id as usize];
            &node.rotulo
        } 
        ...
        pub fn peso(&self, u: i32, v: i32) -> i32{
            let edges = self.adj.get(&u).unwrap();
            for edge in edges {
                if edge.neighbor == v {
                    return edge.weight;
        ...
        //  Cria um grafo NÃO DIRIGIDO a partir de um arquivo, Time O(n) Space O(n), n é número de arestas.
        pub fn ler(filename: &String) -> WeightedGraph<String>{
            ...
            let mut my_graph: WeightedGraph<String> =  WeightedGraph::new(false);
            let mut l = 0;
        ...
        pub fn arestas(&self) -> Vec<(i32, i32, i32)> {
